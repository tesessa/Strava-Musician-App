...
function isLikeFailure(r: unknown): r is { error: string; status: number } {
  return typeof r === "object" && r !== null && "error" in r && "status" in r;
}

// POST /practice-logs/:practiceLogId/likes
export async function likePracticeLog(
  req: Request,
  { params }: { params: { practiceLogId: string } },
) {
  ...
  const canAccess = await likesService.canUserAccessPracticeLog(
    user.userId,
    practiceLogId,
  );
  if (!canAccess) {
...
export async function unlikePracticeLog(
  req: Request,
  { params }: { params: { practiceLogId: string } },
) {
  const { user, error } = await authenticateToken(req);
  if (error) return error;
  const { practiceLogId } = params;
  const result = await likesService.unlikePracticeLog(
    user.userId,
    practiceLogId,
  );
  if (isLikeFailure(result)) {
...
export async function getPracticeLogLikes(
  req: Request,
  { params }: { params: { practiceLogId: string } },
) {
  const { user, error } = await authenticateToken(req);
