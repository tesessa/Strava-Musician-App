...
export async function createComment(req: Request, practiceLogId: string) {
  ...
  const canAccess = await commentsService().canUserAccessPracticeLog(user.userId, practiceLogId);
  if (!canAccess) return NextResponse.json({ error: "forbidden" }, { status: 403 });
  const body = await req.json();
  if (!body.text || typeof body.text !== "string") {
    return NextResponse.json({ error: "text_required" }, { status: 400 });
  }
  const result = await commentsService().createComment(user.userId, practiceLogId, body.text);
